...
import { resourceDescription } from "../resource-translations";
...
export default function ChatPage() {
  ...
  function listen() {
    ...
    if (!SpeechRecognition) { setError(t.noVoice); return; }
  ...
  return <main className="chat-page">
    ...
    <header><div className="eyebrow">LAKE COUNTY · ILLINOIS</div><h1>{t.title}</h1><p>{t.intro}</p></header>
    <section className="chat-card" aria-label="Resource chat">
      <div className="chat-messages" aria-live="polite">{messages.length === 0 && <div className="from-helper"><p>{welcome[lang]}</p></div>}{messages.map((item, index) => <div key={index} className={item.role === "user" ? "from-user" : "from-helper"}>
        <p>{item.text}</p>
        {item.role === "assistant" && <><button className="read-aloud" onClick={() => speak(item.text)}>🔊 {t.read}</button>{item.resources?.map(resource => <article className="chat-resource" key={resource.id}><strong>{resource.name}</strong><span>{resourceDescription(resource.id,resource.description,lang)}</span><div><a href={`tel:${resource.phone.startsWith("211") ? "211" : resource.phone.replace(/[^\d]/g, "")}`}>{t.call} {resource.phone}</a><a href={resource.website} target="_blank" rel="noopener noreferrer">{t.website} ↗</a></div></article>)}</>}
      </div>)}{busy && <div className="from-helper"><p>{t.finding}</p></div>}</div>
      <div className="chat-suggestions" aria-label="Suggested questions">{t.suggestions.map(prompt => <button key={prompt} onClick={() => send(prompt)} disabled={busy}>{prompt}</button>)}</div>
