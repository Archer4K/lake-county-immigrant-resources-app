...
import { languages, locale, type Language } from "./locale";
import { hindiNav } from "./hindi";
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try { saved = window.localStorage.getItem("lake-resource-language"); } catch {}
    if (choices.includes(saved as Language)) setLanguage(saved as Language);
    else setNeedsChoice(true);
    setReady(true);
  }, []);
  useEffect(() => { document.documentElement.lang = locale[language].split("-")[0]; }, [language]);
  function choose(value: Language) {
    setLanguage(value);
    try { window.localStorage.setItem("lake-resource-language", value); } catch {}
    if ("speechSynthesis" in window) window.speechSynthesis.cancel();
    setNeedsChoice(false);
  }
  return <Context.Provider value={{ language, choose }}>
    {ready ? children : <div role="status" style={{padding:24}}>Loading · Cargando · Загрузка · लोड हो रहा है…</div>}
    {ready && needsChoice && !pathname.startsWith("/admin") && <div className="language-overlay"><section className="language-dialog" role="dialog" aria-modal="true" aria-labelledby="choose-language-title"><span className="mark" aria-hidden="true">✳</span><h2 id="choose-language-title">Choose your language<br/><span>Elija su idioma · Выберите язык · अपनी भाषा चुनें</span></h2><p>{text.English.detail}</p><div className="language-choices">{choices.map(choice => <button key={choice} onClick={() => choose(choice)} lang={locale[choice].split("-")[0]}>{choice}<span aria-hidden="true">→</span></button>)}</div></section></div>}
  </Context.Provider>;
}
